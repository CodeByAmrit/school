...
student.get('/', checkAuth, async (req, res) => {
    try {
        ...
        const studentClass = req.headers["studentclass"] || req.headers["Studentclass"];
        ...
        const [results] = await connection.execute(query, [teacher_id, studentClass]);
...
student.post('/', checkAuth, async (req, res) => {
    try {
        const teacher_id = req.user._id;
        const { name, father_name, mother_name, session, class: studentClass, school_id } = req.body;
        let connection = await getConnection();

        const query = `INSERT INTO students (teacher_id, name, father_name, mother_name, session, class, school_id) 
                       VALUES (?, ?, ?, ?, ?, ?, ?)`;
        const [result] = await connection.execute(query, [teacher_id, name, father_name, mother_name, session, studentClass, school_id]);
...
student.put('/:id', checkAuth, async (req, res) => {
    try {
        ...
        const [result] = await connection.execute(query, [name, father_name, mother_name, session, studentClass, school_id, studentId, teacher_id]);
