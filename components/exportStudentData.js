    ...
    `);
        ...
        for (const student of students) {
            ...
            if (student.student_sign) {
                fs.writeFileSync(path.join(studentDir, 'signature.jpg'), student.student_sign);
            }
            ...
            for (const file of pdfFiles) {
                const safeFileName = file.file_name?.replace(/[^a-zA-Z0-9_\-\.]/g, '_') || `document_${Date.now()}.pdf`;
