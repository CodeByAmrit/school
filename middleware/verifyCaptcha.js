...
const verifyCaptcha = async (req, res, next) => {
  ...
  if (!captcha) {
    return res
      .status(400)
      .json({ status: 'captcha_failed', error: 'CAPTCHA token missing' });
  }
  ...
  try {
    ...
    if (!response.tokenProperties.valid) {
      ...
      return res
        .status(400)
        .json({ status: 'captcha_failed', error: 'Invalid CAPTCHA token' });
    }

    if (response.tokenProperties.action !== 'LOGIN') {
      return res
        .status(400)
        .json({ status: 'captcha_failed', error: 'Invalid CAPTCHA action' });
    }
    ...
    return res
      .status(500)
      .json({ status: 'error', error: 'Server error verifying CAPTCHA' });
  }
};
